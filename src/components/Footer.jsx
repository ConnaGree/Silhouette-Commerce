  ...
  return (
    <div className="md:px-[5rem] mt-[1rem] px-[2rem] py-[3rem] lg:mt-[3rem] bg-black text-white flex flex-col md:flex-row justify-around">
      <div className="left__section gap-[2rem] mb-[1.5rem] lg:mb-0">
        <h2 className="title text-[1.2rem] font-[600] mb-[1.5rem]">Silhouette</h2>
        <a href="" className="subscribe text-[.9rem] mb-[1.5rem]">Subscribe</a>
        <p className="text-[.7rem] text-[#FAFAFA] my-[1.5rem]">Get 10% off your first order</p>
        <label
      ...
      {footerLinks.map((item, index) => (
        <div key={index}>
          ...
          {item.links.map((subItem, subIndex) => (
            <a key={subIndex} className="block text-[.7rem] mb-2 text-[#FAFAFA]" href={subItem.link}>
              {subItem.name}
            </a>
          ))}
        </div>
      ...
      <div className="right__section">
        ...
        <div className="gap-1 mt-[1rem] flex">
          <span className="qrCode"><img src={QrCode} alt="QrCode" /></span>
          <div>
              <img src={android} className="block object-fit mb-[.5rem]" alt="android download" /><img className="block" src={appstore} alt="apple download" />
          </div>
        </div>
