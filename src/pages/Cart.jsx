  ...
  return (
    <div className="md:px-[5rem] mt-[1rem] px-[2rem] py-[2rem] lg:mt-[3rem]">
      <div className="top_c flex items-center justify-between mb-[2rem]">
        ...
        <div className="cart__header w-[80%] border-b-[1px]">
      ...
      {cartItems.length === 0 ? (
        ...
        <ul className="cart__item-container">
          ...
          {cartItems.map((item, index) => (
            <div className="cart__item flex items-center justify-between bg-[#f9fbfd] my-[1rem] rounded-[.5rem] md:px-4 md:py-3 p-2" key={index}>
              <div className="flex items-center gap-2">
                <img src={item.img} className="w-[60px] h-[60px] object-contain rounded-[0.3rem]" alt={item.title} />
                <span className="text-[.8rem]">{item.title}</span>
              </div>

              <span className="price text-[.7rem]">Br. {item.regularPrice}</span>

              <input className="border-[1px] rounded-[.5rem] px-4 py-1 text-[.7rem] items-center text-inherit" type="number" />

              <span className="subtotal text-[.7rem]">Br. {item.regularPrice}</span>
            </div>
          ))}
        </ul>
      )}
